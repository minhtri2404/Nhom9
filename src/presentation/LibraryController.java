...
public class LibraryController implements ActionListener, ListSelectionListener {
        ...
        try {
            ...
            if (view.getPriceField().getText().isEmpty() || publisher.isEmpty()
                ...
                if (view.getTypeComboBox().getSelectedItem().equals("Sách giáo khoa")) {
                    ...
                    Command command = new AddCommand(view.getLibraryService(), newTextBook);
                    command.execute();
                    ...
                    Command command = new AddCommand(view.getLibraryService(), newBook);
                    command.execute();
                }
    ...
    public void refreshBookTable() {
        ...
        List<Book> books = view.getLibraryService().getAllBooks();
        for (Book book : books) {
    ...
    public void searchPublisher() {
        ...
        if (publisherStr != null && !publisherStr.isEmpty()) {
            List<Book> books = view.getLibraryService().getBooksByPublisher(publisherStr);
        ...
        if (selectedRow != -1) {
            try {
                ...
                if (view.getTypeComboBox().getSelectedItem().equals("Sách giáo khoa")) {
                    ...
                    view.getLibraryService().updateBook(textBook);
                } else {
                    ...
                    view.getLibraryService().updateBook(referenceBook);
                }
                clearInputFields();
            } catch (NumberFormatException ex) {
    ...
    public void showSelectedBookInfo() {
        ...
        if (selectedRow != -1) {
            ...
            Book book = view.getLibraryService().findBook(bookId);
            if (book != null) {
    ...
    public void removeBook() {
        ...
        if (selectedRow != -1) {
            ...
            if (response == JOptionPane.YES_OPTION) {
                view.getLibraryService().removeBook(bookId);
                refreshBookTable();
                clearInputFields();
            }
        } else {
    ...
    public void getTotalPriceTextBook() {
        double total = view.getLibraryService().getTotalPriceTextBook();
        view.getPriceTextBooksField().setText(total + "");
    }

    public void getTotalPriceRefrenceBook() {
        double total = view.getLibraryService().getTotalPriceRefeneBook();
        view.getPriceReferenceBooksField().setText(total + "");
    }

    public void getAvgPriceRefrenceBook() {
        double total = view.getLibraryService().getAvgPriceRefeneBook();
